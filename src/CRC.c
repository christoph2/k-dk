/*!file CRC.C
 *
 * Utility-Functions for 16Bit-CRC Lookup/Calculation.
 *
 */

/*
**
**  Hinweis: Naming-Conventions evtl. 'UtlCRC' ???
**
*/
...
//#define GENPOLY ((uint16)0x2021)
#define GENPOLY ((uint16)0xa001)
...
void CRCGenerateTable(void)
{
    ...
    for (j=0;j<=31;j++) {
        fprintf(out,"\n\t");
            for (i=8*j;i<8*j+8;i++) {
                fprintf(out,"0x%04x,\t",CRCCalculate(GENPOLY,i,0));
            }
    }
    fprintf(out,"\n};");
    fclose(out);
